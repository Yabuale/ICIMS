...
const ClerkRequest = () =>{  
    ...
    useEffect(() => {
        const fetchWantedCriminal = async () => {
          try {
            ...
            if (token) {
              setToken(token)
              const response = await axios.get(`http://127.0.0.1:8000/clerk/requests/`, {
                headers: {
      ...
      if (!req || req.length === 0) {
        ...
        <div className="w-full lg:w-1/2 flex flex-col items-center justify-center lg:px-2 xl:px-0 text-center">
            ...
            <p className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-wider text-gray-300 ">No Requests</p>
            <p className="text-lg md:text-xl lg:text-2xl text-gray-500 my-12">You don't have any requests at the moment</p>
   ...
   return(
      <>
      {req.map((r,index) =>(
<div  key={index} className="ml-28 mt-10">
   <div className="flex max-w-md overflow-hidden bg-white rounded-lg shadow-2xl ">
    <div className="w-2/3 p-4 md:p-4">
        <h1 className="text-xl font-bold text-gray-800 ">{r.name} {r.fname}</h1>
        ...
        <div className="flex justify-between mt-3 item-center">
            ...
            <NavLink to={`/clerk/reqdetail/${r.id}`} className="px-4 py-3 text-xs font-bold text-white uppercase transition-colors duration-300 transform bg-sky-800 rounded  hover:bg-sky-700  focus:outline-none focus:bg-sky-700 ">View</NavLink>
        </div>
    </div>
</div>
</div>
