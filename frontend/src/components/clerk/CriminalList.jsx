...
const CriminalList = () => {
    ...
    useEffect(() => {
      ...
      const handledelete = async (e) => {
       ...
       const url2 =`http://127.0.0.1:8000/criminals/criminaldetail/${id}/`
      ...
      if (!wantedCriminal || wantedCriminal.length === 0) {
        ...
        <div className="w-full lg:w-1/2 flex flex-col items-center justify-center lg:px-2 xl:px-0 text-center">
            ...
            <p className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-wider text-gray-300 ">No criminal</p>
            <p className="text-lg md:text-xl lg:text-2xl text-gray-500 my-12">Seems like there you didn't save any  criminal record.</p>
...
<section className="container px-4  mx-24      shadow-2xl bg-sky-50 p-10 rounded-2xl w-4/5  md:w-9/12 lg:w-4/5">
    <div className="flex items-center gap-x-3">
        <h2 className="text-lg font-medium text-sky-950 ">Wanted</h2>

        <span className="px-3 py-1 text-xs text-sky-950 bg-sky-200 rounded-full  ">100 Posted</span>
    </div>
                    ...
                    <table className=" w-full  ">
                        <thead className="bg-sky-700  ">
                            <tr>
                                ...
                                <th scope="col" className="px-4 py-3.5 text-sm font-normal text-left rtl:text-right text-white ">date of birth</th>

                                <th scope="col" className="px-4 py-3.5 text-sm font-normal text-left rtl:text-right text-white ">gender</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200 ">
                           ...
                           {wantedCriminal.map((wanted,index) =>(
...
<td className="px-4 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
    <div className="inline-flex items-center gap-x-3">
        ...
        <div className="flex items-center gap-x-2">
            ...
            <div>
                <h2 className="font-medium text-gray-800  ">{wanted.first_name} {wanted.last_name}</h2>
...
<td className="px-12 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
    <div className="inline-flex items-center px-3 py-1 rounded-full gap-x-2  ">
        ...
        <h2 className="text-sm font-normal ">{wanted.height}</h2>
    </div>
</td>
<td className="px-4 py-4 text-sm text-gray-500  whitespace-nowrap">{wanted.nationality}</td>
<td className="px-4 py-4 text-sm text-gray-500  whitespace-nowrap">{wanted.date_of_birth}</td>
...
<td className="px-4 py-4 text-sm whitespace-nowrap">
    <div className="flex items-center gap-x-6">
        ...
        {showdelModal && <ConfdelModal showdelModal={showdelModal} isLoading={isLoading} message={'you are about to delete a record, This process cannot be undone'} clickbutton={'Delete'} onClose={() => showdeleteModal(false)} />}
        ...
        <NavLink to={`/clerk/edit/${wanted.id}`} className="text-gray-500 transition-colors duration-200  hover:text-yellow-500 focus:outline-none">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5">
