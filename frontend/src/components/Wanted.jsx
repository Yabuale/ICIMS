...
const Wanted = (props) => {
  const items = props.wanted;

  if (!items || items.length === 0) {
    return <>
    <div className="w-full h-screen flex flex-col lg:flex-row items-center justify-center space-y-16 lg:space-y-0 space-x-8 2xl:space-x-0 bg-sky-50">
    <div className="w-full lg:w-1/2 flex flex-col items-center justify-center lg:px-2 xl:px-0 text-center">
        ...
        <p className="text-lg md:text-xl lg:text-2xl text-gray-500 my-12">Seems like there is no wanted criminal at the moment. Thank you for trying our community safe</p>
      ...
      <div className="flex flex-wrap -m-4">
        {items.map((item, index) => (
          <WantedCard
            ...
            imageUrl={item.imageUrl}  // Replace with the actual property name in your item object
            name={item.name} 
            description={item.description}
            last_known_location={item.last_known_location}
            age={item.age}
            criminal_offenses={item.criminal_offenses}
            id={item.id}
            alias={item.alias}
