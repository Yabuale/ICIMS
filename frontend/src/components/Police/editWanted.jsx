...
const EditWanted = () =>{
 ...
 const [last_known_location, setLast_known_location] = useState('Loading...')
 const [nationality, setNationality] = useState('Loading...')
 const [criminal_offenses, setCriminal_offenses] = useState('Loading...')
 const [reward, setReward] = useState(0.00)
 const [contact_information, setContact_information] = useState('Loading...')
 ...
 useEffect(() => {
   ...
   const fetchWantedCriminal = async () => {
     try {
       const response = await axios.get(`http://127.0.0.1:8000/police/wanteddetail/${wantedId}/`);
       if(response.status === 200){
       setWantedCriminal(response.data);
       setAge(response.data.age)
       setName(response.data.name)
       setDescription(response.data.description)
       setContact_information(response.data.contact_information)
       setReward(response.data.reward)
       setNationality(response.data.nationality)
       setAlias(response.data.alias)
       setLast_known_location(response.data.last_known_location)
       setCriminal_offenses(response.data.criminal_offenses)
       
       }
       else{
        navigate("/police/list")
       }
     } catch (error) {
...
const url = `http://127.0.0.1:8000/police/wanteddetail/${wantedId}/`
 ...
 <br/>
  ...
  <div className="grid  mx-24 lg:mx-auto   bg-sky-50   rounded-lg shadow-xl w-4/5  md:w-9/12 lg:w-1/2">
    ...
    <div className="flex justify-center">
      <div className="flex">
        <h1 className="text-gray-600 font-bold md:text-2xl text-xl">Edit Wanted</h1>
      </div>
    </div>
    ...
    <div className='flex items-center justify-center  md:gap-8 gap-4 pt-5 pb-5'>
      ...
      {showconfModal && <ConfModal showconfModal={showconfModal} isLoading={isLoading} message={'you are about to update a wanted criminal please conform by clicking Update'} clickbutton={'Update'} onClose={() => showconformModal(false)} />}
